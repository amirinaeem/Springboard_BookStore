...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const ext = fileUrl.endsWith(".epub")
      ? ".epub"
      : fileUrl.endsWith(".pdf")
      ? ".pdf"
      : "";
    const safeName =
      (book.title || "book").replace(/[^\w.-]+/g, "_") + ext;
    ...
    if (req.method === "HEAD") {
      return new Response(null, {
        ...
        headers: {
          ...
          "Access-Control-Allow-Origin": "*", // 👈 fixes epub.js CORS issue
        },
      });
    }
    ...
    headers.set("Access-Control-Allow-Origin", "*"); // 👈 required for epub.js
