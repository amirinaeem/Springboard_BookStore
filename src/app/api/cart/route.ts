...
async function getOrCreateCart(userId: string) {
  ...
  return prisma.cart.create({ data: { userId }, include: { items: { include: { book: true } } } });
}
...
export async function POST(req: Request) {
  ...
  let dbBook = null;
  if (fromGoogle && book) dbBook = await ensureBookSaved(book);
  else if (bookId) dbBook = await prisma.book.findUnique({ where: { id: bookId } });
